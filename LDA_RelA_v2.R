...
for (i in 1:10){
  ...
  #-------------------------------------------------------------------------------
  #FIT LDA MODEL
  model <- lda(ligand~., data=train)
  model
  #-------------------------------------------------------------------------------
  # Verify results
  conf <- table(list(predicted=predict(model,test)$class, observed=test$ligand))
  # save the confusion matrix in .CSV file
  filename = (paste("T://LMBI//021Transcription_Systems_Dynamics_and_Biology//Toufiq//R studio project//DKin project2//Confusion matrix//confusion matrix 10fold_CV//Confusion_LDA_RelA_",i,".CSV",sep=""))
